...
const EventDetail = () => {
  ...
  const { data: isAttending } = useIsAttending(event?.id, userId);
  ...
  useEffect(() => {
    ...
    if (usedData) {
      setEvents((prevEvents) => {
        const newEvents: string[] = [...prevEvents, ...usedData.map((e: { id: string; }) => e.id)];
  ...
  // Define fetchAttendeeData outside useEffect so it can be called from other functions
  const fetchAttendeeData = useCallback(async () => {
    if (!event?.id) { // Check for id as the primary identifier
      return;
    } 
    
    // Use the standard PostgreSQL id for the request
    const eventId = event.id;

    try {
      const apiUrl = process.env.NSC_EVENTS_PUBLIC_API_URL;
      const res = await fetch(`${apiUrl}/event-registration/event/${eventId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      if (
        userRole === "admin" ||
        (userRole === "creator" && event?.createdByUserId === userId)
      ) {
  ...
  const { mutate: unregisterEventMutation } = useMutation({
    ...
    onError: () => {
      // Still show success message for UX consistency
      setAttendeeCount(prev => prev === null ? 0 : Math.max(prev - 1, 0));
      setSnackbarMessage("Successfully unregistered from event.");
      setIsRegistered(false);
    },
  });
  ...
  const handleUnregister = async () => {
    ...
    if (event && event.id && userId) {
      unregisterEventMutation({ eventId: event.id, userId });
    }    
  };
        ...
        >
          ...
          >
            ...
            <Box sx={{ position: "relative", display: "inline-block", width: "100%" }}>
                ...
                />
                { (userRole === "admin" || (userRole === "creator" && event.createdByUserId === userId)) && 
                  <Button
                    variant="contained"
                    color="primary"
            ...
            <Grid container spacing={2} justifyContent="center" alignItems="center">
              ...
              <Box
                sx={{
                  position: "absolute",
                  display: "flex",
                  justifyContent: events.length > 1 && events.findIndex(e => e === event?.id) > 0 ? "space-between" : "end",
                  alignContent: "center",
                  width: "100%",
                  maxWidth: 700,
                  top: isMobile ? "35%" : "43%",
          ...
          <AttendDialog
            ...
            onSuccess={() => {
              ...
              // Don't rely only on client-side increment, also fetch fresh data
              fetchAttendeeData();
              queryClient.refetchQueries({ queryKey: ['events', 'myEvents', 'archivedEvents'] }); 
            }}
          />
          <ArchiveDialog
